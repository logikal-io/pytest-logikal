...
def test_run_invalid(plugin_item: Callable[..., Item]) -> None:
    ...
    with raises(ItemRunError) as error:
        item.runtest()
    error.match(r'Missing semicolon.*\(semi\)')
    error.match(r'Expected indentation.*\(indent\)')
    error.match(r'Expected multiple line comments.*\(multiline-comment-style\)')
    error.match(r'This line has a length of.*\(max-len\)')
    error.match(r'Strings must use singlequote.*\(quotes\)')
    error.match(r'Expected no linebreak.*\(nonblock-statement-body-position\)')
